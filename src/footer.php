<div style="font-size:14px;clear:both;"><div class="menu-footer" style="clear:both;">
  <a href="blogs.php">блог</a>
</div>
<p class="about-footer" style="font-size:11px;clear:both;">OpenVK Classic. Лицензия: <a href="LICENSE.txt">GNU General Public License v3.0</a>.<br><?php echo date("Y",time()); ?> Veselcraft</p></div>
<br><script type="text/javascript" src="js/jquery/jquery.js"></script>
	<script type="text/javascript" src="js/jquery/jquery-ui.js"></script>
	<script type="text/javascript" src="js/jquery/window/jquery.window.js"></script>
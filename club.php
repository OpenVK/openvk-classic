...
?>
<?php if ($club['closed'] == '2' and $club['authorid'] == $_SESSION['id']) {?>
типа соси хуй
<?php } ?>
<?php if ($club['closed'] != '2') { ?>
<div id="content-infoname"><b><?php if ($club['ban'] == '1') {
    echo "Сообщество заблокированно";
} elseif ($club['verify'] > 0) {
    echo "Официальное сообщество";
} else {
    echo "Сообщество";
} ?></b><?php if ($club['verify'] == '1') {
    echo '<img src="img/verify_silver.svg" width="12" height="12" style="margin-left:5px;margin-right:5px;margin-bottom:-2px;" onmouseenter="openVerify();" onmouseleave="openVerify();"><div id="verify" style="display:none;margin:10px 0;">Данная группа была официально верифицирована администрацией OpenVK.</div>';
}  if ($club['verify'] == '2') {
    ...
    echo '<img src="img/verify_green.svg" width="12" height="12" style="margin-left:4px;margin-right:4px;margin-bottom:-2px;" onmouseenter="openVerify();" onmouseleave="openVerify();"><div id="verify" style="display:none;margin:10px 0;">Данная группа - группа от команды разработчиков.</div>';
}?>
    </div>
  <?php if ($club['ban'] == "1") { ?>
...
<br><br><?php  if ($club['closed'] == '0' or $chsubcl['id1'] == $_SESSION['id'] and $chsubcl['id2'] == $id) { ?>
   ...
   <div id="content-wall-post-avatar"><img id="avatar" src="'.$ava.'" width="50"></div>'.$onlinewall.'</td><td style="width:345px;vertical-align:0;">
     <div id="content-wall-post-infoofpost">
      ...
      <div id="content-wall-post-authorofpost"><text style="margin-right: 3px;"><b><a href="club'.$club['id'].'">'.$club['name'].'</a></b></text><br><div id="content-date"><a href="gpost'.$wall['id'].'">'.zmdate($wall['date']).'</a></div></div>
     
     <div id="content-wall-post-text">'.$wall['text'].$im.'<br></div>
     </div>'.$deletebutton.'<a href="gpost'.$wall['id'].'" style="float:right;">Открыть комментарии</a><div style="clear:both;"></div>
...
<?php if ($club['authorid'] != "0" and $club['type'] == '0') {?>
